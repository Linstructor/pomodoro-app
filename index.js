...
app.on("ready", () => {
  ...
  window.webContents
    .executeJavaScript("window.localStorage.getItem('alwaysOnTop')")
    .then(result => {
  ...
  ipcMain.on("end", (event, end = false, params = {}) => {
    ...
    if (end) {
      new Notification({
        title: `Take a ${
          params.max === params.current ? "long" : "short"
        } break`,
        body: `Pomodoro phase ${params.current - 1} done, ${
          params.max === params.current ? "25" : "5"
